...
export class CreateValidator implements HandlerImplementation {
  ...
  public validate(query: {
    ...
    const requestQuery = query.data.toString().trim();
    const regExp =
      /^\s*?CREATE TABLE ([A-Za-z]\w+)\s*?\(\s*?([A-Za-z]\w+) (INTEGER|FLOAT) (NOT NULL)? (PRIMARY KEY)?(\s*?,\s*?([A-Za-z]\w+) (INTEGER|FLOAT|VARCHAR|LONG TEXT|BOOLEAN|DATE)( NOT NULL| NULL)?( DEFAULT ("\w+"|-?\d+(\.\d{1,3})?|TRUE|FALSE|NULL|NOT NULL))?)*?\s*?\);$/;
