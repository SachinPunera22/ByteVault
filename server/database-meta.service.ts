...
export class DatabaseMetaService {
  ...
  public async saveMeta() {
     const string =
      "Name:Bytevault\nversion:1\nNumberOfTables:2\nencoding:utf-8\n\nname:users\npath:tables/users/users.ibd\nname:roles\npath:tables/roles/roles.ibd";
  ...
  public async readMeta() {
    const fd = await fs.open("./server/storage/database.bin", "r");
    ...
    const basicMeta = this.convertToBasicMetaObject(basicMetaString);
    const tablesMeta = this.convertToTablesMetaObject(tablesMetaString);
    basicMeta.tables = tablesMeta;
    return basicMeta;
  }
